    ...
    def forward(self, backbone_output: BatchFeature, action_input: BatchFeature) -> BatchFeature:
        ...
        with torch.amp.autocast(dtype=torch.bfloat16, device_type=torch.device(self._device).type):
            ...
            if actions.numel() > 0:
                ...
                # has_real_action is shape (B,); action_loss_per_sample is (B, 96, action_dim).
                # (B, 1, 1) broadcasts cleanly to (B, 96, action_dim); the previous (B, 1)
                # only worked at B=1 (where every dim is 1) and crashes for batch>1.
